...
$queryRejectedPro=$conn->prepare("SELECT * FROM creditdetails WHERE id=?");
...
$UpdateReject=$conn->prepare("UPDATE `creditdetails` SET `productDetails`=?,`rejectedProducts`=?,`productPrice`=?,`firstPrice`=?,`remindMoney`=?,`avaragePay`=?,`rejectId`=?,`rejectName`=? WHERE id=?");
$UpdateReject->execute([$pro,$newRejected,$malinQiymeti,$ilkinOdenis,$remindMoney,$avaragePay,$uid,$rejectExecName,$mid]);

$queryNew=$conn->prepare("SELECT * FROM `bmk` WHERE bmkNo=? AND sellStyle=?");
$queryNew->execute([$mid,1]);
...
$saveNews->execute([$_SESSION["id"],7,date("d.m.Y"),date("H:i:s"),$descriptionNews]);
?>
