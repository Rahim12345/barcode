...
$queryRejectedPro=$conn->prepare("SELECT * FROM topdan WHERE id=?");
...
$UpdateReject=$conn->prepare("UPDATE `topdan` SET `productDetails`=?,`rejectedProducts`=?,`price`=?,`rejectId`=?,`rejectName`=? WHERE id=?");
...
$saveNews->execute([$_SESSION["id"],9,date("d.m.Y"),date("H:i:s"),$descriptionNews]);
...
<?php
echo '<meta http-equiv="refresh" content="0;url=index.php?id=38&mid='.$mid.'">';
}
?>