...
if($timer>0)
{
$saveDetails=$conn->prepare("INSERT INTO `cash`(`name`, `address`, `telno`, `timer`, `productDetails`, `price`, `date`, `given`, `sellerID`, `sellerName`,`rasot`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
...
$query4CahsID=$conn->prepare("SELECT * FROM cash WHERE id>? ORDER BY id DESC");
...
$saveNagdBmk=$conn->prepare("UPDATE `cash` SET `realbmk`=? WHERE id=?");
$saveNagdBmk->execute([$BmkID,$CahsID]);
...
$saveNews->execute([$_SESSION["id"],3,date("d.m.Y"),date("H:i:s"),$description]);
...
include 'updateProducts4Selled4Nagd.php';
?>