...
if(isset($_POST["Pay"]))
{
  ...
  {
    include 'promiseTPayInserAssist.php';
  }
  else
  {
...
else
{
  ...
  elseif($rows4CashPay[0]["price"]>$cashPay)
  {
    ...
    $updatePaid=$conn->prepare("UPDATE `topdan` SET `price`=?,`qaliqiOdeyen`=? WHERE id=?");
  ...
  else
  {
    ...
    $updatePaid=$conn->prepare("UPDATE `topdan` SET `price`=?,`qaliqiOdeyen`=?,rasot=? WHERE id=?");
...
$saveNews->execute([$_SESSION["id"],6,date("d.m.Y"),date("H:i:s"),$description]);
