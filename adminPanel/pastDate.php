    ...
    <div class="card-body">
	   <table class="table table-dark" style="width: 100%;">
		  <thead class="bg-success">
	    	<tr>
	    		<th scope="col" colspan="3"  style="text-align: center;">Keçmiş günə kredit yazmağa icazə ver</th>
	    	</tr>
		    <tr>
		      <th scope="col">№</th>
		  	...
		  	if(isset($_POST["permit"]))
		  	{
		  		...
		  		if(isset($_POST['permitInputs']))
		  		{
					...
					if($count4Workers!=0)
					{
						...
						$UpdatePermit=$conn->prepare("UPDATE `users` SET `krediPastDatePermit`=? WHERE  id=?");
		  			...
		  			include 'pastDate_default.php';
		  		}
		  		else
		  		{
		  			$UpdatePermit=$conn->prepare("UPDATE `users` SET `krediPastDatePermit`=? WHERE  status!=?");
