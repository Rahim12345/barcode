...
<form action="" method="POST">
	...
	<select class="custom-select my-1 mr-sm-2" id="store" name="store" >
		...
		{
			if($store==$row4StoreInworker["name"])
			{
		?>
			<option value='<?php echo $row4StoreInworker["name"]; ?>' selected><?php echo $row4StoreInworker["name"]; ?></option>
		<?php
			}
			else
			{
	...
	<select class="custom-select my-1 mr-sm-2" id="jobs" name="job" >
		...
		{
			if($job==$row4JobsInworker["name"])
			{
		?>
			<option value='<?php echo $row4JobsInworker["name"]; ?>' selected><?php echo $row4JobsInworker["name"]; ?></option>
		<?php		
			}
			else
			{
	...
	<div class="form-group row">
		...
		<div class="col-sm-10">
		  <input type="text" name="name" class="form-control" id="name" value="<?php echo $name; ?>" placeholder="Rahim" maxlength="51">
		</div>
	</div>
	<div class="form-group row">
		<label for="password" class="col-sm-2 col-form-label">Şifrə<sup>*</sup></label>
		<div class="col-sm-10">
		  <input type="password" name="password" class="form-control" id="password" value="<?php echo $password; ?>"placeholder="8-40 simvoldan ibarət bir şifrə" maxlength="51">
		</div>
	</div>
	<div class="form-group row">
		<div class="col-sm-10">
